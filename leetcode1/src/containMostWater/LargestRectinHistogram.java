...
public class LargestRectinHistogram {
	public static int largestRectinHistogram(int[] height) {
		int res = 0;
		for (int i = 0; i < height.length; i++) {
			...
			for (int j = i; j >= 0; j--) {
				minheight = Math.min(minheight, height[j]); //Different from ContainMostWater, the rectangle area depends on the smallest number in range.
	...
	public static int largestRectinHistogram_1(int height[]) {
		int res = 0;
